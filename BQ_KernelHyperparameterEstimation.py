...
#------------------------------------- Define Kernels & corresponding Gram Matrix --------------------------------------
# We provide two ways to define the kernels and kernel matrix. When the aim is to estimate a >1D integral, use the
#   first collection of function, otherwise when the aim is to estimate a 1D integral used the second collection.
...
#     x = np.asarray(x).ravel()
#     y = np.asarray(y).ravel()
#     Norm = np.sqrt((x - y).T @ (x - y))  # Euclidean norm of x - y
...
#     # y = np.asarray(y).reshape(1, -1)  # (1,m)
#     # return sigmaf2_Poly * ((x *  y) + c_Poly) ** 2
#     return sigmaf2_Poly * ((x.T @ y) + c_Poly)**2
#
# def Kernel_Periodic(x, y, lmbd_P, l_P, sigmaf2_P, **_):
#     x = np.asarray(x).ravel()
#     y = np.asarray(y).ravel()
...
#
# ### Gram Matrix
# def KernelMatrix(KernelFunction, x, y, **parameters):  # For general (nxd)-dim arrays
#     x = np.asarray(x)
#     y = np.asarray(y)
#
#     n = x.shape[0]
...
def LogMarginalLikelihood(loghyperparameters, X_Input, y, PriorMeanVector, KernelFunction, ParameterNames):
    ...
    #------------------------------ Gram Matrix -------------------------------
    K_XX = KernelMatrix(KernelFunction, X_Input, X_Input, **kernel_parameters)
...
def MLEHyperparameters(initial_parameters, X_Input, y, PriorMeanVector, KernelFunction, ParameterNames):
    ...
    FindingOptimizer = minimize(
        ...
        bounds = [(np.log(1e-2), np.log(1e2))] * len(initial_parameters)
    ...
    #---------------------------- Retransform Hyperparameters ----------------------------
    OptimalLogHyperparameters = FindingOptimizer.x
